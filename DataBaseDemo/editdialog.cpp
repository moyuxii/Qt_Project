...
int uniqueCarid;
int uniqueFactoryid;
EditDialog::EditDialog(QSqlRelationalTableModel *cars,
                       QSqlTableModel *factory,
...
void EditDialog::submit()
{
    ...
    if (factory.isEmpty()|| address.isEmpty() ||name.isEmpty ()){
        ...
        int factoryId = findFactoryTd(factory);
        if(factoryId==-1){
...
int EditDialog::findFactoryTd(const QString &factory)
{
    int row=0;
